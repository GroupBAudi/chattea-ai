...
def evaluate_model(model, X_val, y_val, label_encoder):
    ...
    with torch.no_grad():
        ...
        print("\n📈 Per-Intent Accuracy:")
        print("-" * 80)
        intent_map = dict(enumerate(label_encoder.classes_))
        
        for intent_id in range(len(intent_map)):
            intent_name = intent_map[intent_id]
            mask = y_val == intent_id
            
            if mask.sum() > 0:
                intent_acc = (preds[mask] == y_val[mask]).float().mean().item()
                count = mask.sum().item()
                ...
                print(f"{status} {intent_name:30s}: {intent_acc:.3f} ({count:2d} samples)")
        ...
        for true_intent, pred_intent, count in confused_pairs[:5]:
            print(f"   {true_intent:25s} → {pred_intent:25s}: {count} times")
...
def main():
    ...
    # Prepare validation data if not already split
    if not os.path.exists(config.MODEL_PATH + ".evaluated"):
        ...
        # Run evaluation
        eval_results = evaluate_model(model, X_val, y_val, le)
        
        # Mark as evaluated
        with open(config.MODEL_PATH + ".evaluated", "w") as f:
            f.write(f"Accuracy: {eval_results['accuracy']:.4f}")
    else:
        print("✓ Model already evaluated (delete .evaluated file to re-run)")
